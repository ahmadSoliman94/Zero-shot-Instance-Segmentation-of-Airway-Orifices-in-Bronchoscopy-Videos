...
class Broncho_Dataset(Dataset):
    def __init__(self, spatial_dim: int = 128, transformer=None, normalize: bool = True, gray_scale: bool = False):
        super(Broncho_Dataset, self).__init__()
        self.transformer = transformer
        self.normalize = normalize
        if normalize:
            warnings.warn('Foreign statistics from phantom is used!')

        base_path = Path('./Raw/Broncho-LC/')
        img_path = base_path.joinpath('img_cropped')
        gt_path = base_path.joinpath('gt_cropped')
        # load data
        self.img = []
        self.gt = []
